...
#define DEFINE_ARRAY(arr_name, type)                                           \
  ...
  inline pybind11::array_t<type> arr_name() const {                            \
    return vector_to_array<type>(this->_##arr_name);                           \
  }
...
private:
  ...
  uint8_t get_nhalves(const HGCROCv2RawData &raw_data) const {
    const uint8_t new_n =
        raw_data.data().size() >= (2 * HGCROC_DATA_BUF_SIZE) ? 2 : 1;
  ...
  uint8_t get_nlinks(const HGCROCv2RawData &raw_data) const {
    const uint8_t new_n =
        raw_data.data().size() - (HGCROC_DATA_BUF_SIZE * this->nhalves);
  ...
  uint8_t get_nchannels(const HGCROCv2RawData &rawdata) const {
  ...
  inline static int get_trigwidth(const HGCROCv2RawData &roc_data) {
    return std::accumulate(roc_data.triglatency().begin(),
                           roc_data.triglatency().end(), 0,
                           [&](uint32_t width, const uint32_t val) {
                             return width + std::bitset<32>(val).count();
                           });
  }

  inline static uint16_t get_bxcounter(const uint32_t header) {
    return (header >> 16) & 0xfff;
  }

  inline static uint8_t get_eventcounter(const uint32_t header) {
    return (header & 0xffff) >> 10;
  }

  inline static uint8_t get_orbitcounter(const uint32_t header) {
  ...
  static int get_corruption(const std::vector<uint32_t> &data) {
    ...
    std::transform(crcvec.begin(), crcvec.end(), crcvec.begin(),
                   [](uint32_t w) {
                     return ((w << 24) & 0xFF000000) | ((w << 8) & 0x00FF0000) |
                            ((w >> 8) & 0x0000FF00) | ((w >> 24) & 0x000000FF);
                   });
    const auto bytes = reinterpret_cast<const unsigned char *>(crcvec.data());
    const auto crc32 =
        boost::crc<32, 0x4c11db7, 0x0, 0x0, false, false>(bytes, 39 * 4);
  ...
  inline static uint8_t get_totflag(const uint32_t word,
  ...
  inline static uint16_t get_toa(const uint32_t word, const uint32_t channel) {
    return word & 0x3ff;
  }
  ...
  inline static uint8_t get_trigger_rawsum(const uint32_t tp,
                                           const uint8_t idx) {
